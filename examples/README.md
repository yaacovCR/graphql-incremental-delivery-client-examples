...
- [`react/`](./react/README.md) starts the fake GraphQL execution from a Client Component and
  consumes subsequent payloads through the async iterable directly.
...
The examples use `examples/common/fakeExecuteIncrementally.ts` instead of route
boilerplate. It accepts the expected GraphQL operation document and returns a
hardcoded initial result plus subsequent incremental payloads. Each framework
example consumes the same async iterable at the boundary that makes sense for
that framework.
